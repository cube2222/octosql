...
// PartitionByHash groups the records by their key hashes, and returns them with an index of partition starts.
func PartitionByHash(records []execution.Record, indices []int) (*intintmap.Map, *array.Uint64, execution.Record) {
	// TODO: Handle case where there are 0 records.
	keyHashers := make([]func(rowIndex uint) uint64, len(records))
	for i, record := range records {
		keyHashers[i] = MakeKeyHasher(record, indices)
	}
	...
	// hashCounts := getHashCounts(records, keyHashers)
	// hashNextEntryPosition, hashIndex := calculateNextEntryPositionAndHashIndex(hashCounts)
	// hashPositionsOrdered := buildHashPositionsOrdered(records, overallRowCount, keyHashers, hashNextEntryPosition)
	...
	return hashIndex, hashesArray, execution.Record{Record: record}
}
...
func buildRecords(records []execution.Record, overallRowCount int, hashPositionsOrdered []hashRowPosition) arrow.Record {
	...
	for columnIndex := 0; columnIndex < columnCount; columnIndex++ {
		...
		for recordIndex, record := range records {
			columnRewriters[recordIndex] = MakeColumnRewriter(recordBuilder.Field(columnIndex), record.Column(columnIndex))
		}
