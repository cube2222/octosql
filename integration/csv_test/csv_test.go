...
func Test_CSV(t *testing.T) {
	...
	allFields := []octosql.VariableName{"p.age", "p.eye_color", "p.id", "p.name"}
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				query: "SELECT * FROM people8 p",
			},

			want: execution.NewInMemoryStream(ctx, []*execution.Record{
				execution.NewRecordFromSliceWithNormalize(allFields, []interface{}{7, "blue", 1, "Bill"}),
				execution.NewRecordFromSliceWithNormalize(allFields, []interface{}{14, "brown", 2, "Jenny"}),
				execution.NewRecordFromSliceWithNormalize(allFields, []interface{}{23, "green", 3, "Adam"}),
				execution.NewRecordFromSliceWithNormalize(allFields, []interface{}{29, "brown", 4, "Kate"}),
				execution.NewRecordFromSliceWithNormalize(allFields, []interface{}{37, "brown", 5, "Lisa"}),
				execution.NewRecordFromSliceWithNormalize(allFields, []interface{}{74, "gray", 6, "Ray"}),
				execution.NewRecordFromSliceWithNormalize(allFields, []interface{}{44, "blue", 7, "Hilary"}),
				execution.NewRecordFromSliceWithNormalize(allFields, []interface{}{27, "brown", 8, "Michael"}),
			}),
		},
		{
			name: "Get some records - FILTER",
			args: args{
				query: "SELECT * FROM people8 p WHERE p.age > 60 OR p.eye_color ='blue'",
			},
		...
		},
		{
			...
			args: args{
				query: "SELECT p.eye_color as color, AVG(p.age) as avg_age FROM people8 p GROUP BY p.eye_color",
			},
