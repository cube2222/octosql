...
use crate::execution::datafusion::{GroupByScalar, create_key, create_row};
use std::sync::{Arc, mpsc};
use arrow::array::{BooleanBuilder, ArrayRef, StringBuilder, Int64Builder};
...
impl StreamJoin {
    pub fn new(source: Arc<dyn Node>,
           source_key_fields: Vec<String>,
           joined: Arc<dyn Node>,
           joined_key_fields: Vec<String>,
    ) -> StreamJoin {
...
impl Node for StreamJoin {
    ...
    fn run(&self, ctx: &ExecutionContext, produce: ProduceFn, meta_send: MetaSendFn) -> Result<(), Error> {
        ...
        let handle1 = std::thread::spawn(move || {
            let res = source.run(&ctx1, &mut |ctx, batch| {
        ...
        let handle2 = std::thread::spawn(move || {
            let res = joined.run(&ctx2, &mut |ctx, batch| {
