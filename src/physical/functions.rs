...
use arrow::compute::kernels::comparison::{lt, lt_eq, eq, gt_eq, gt};
...
lazy_static! {
    pub static ref BUILTIN_FUNCTIONS: HashMap<&'static str, Arc<dyn Fn(Vec<Arc<dyn Expression>>)-> Arc<FunctionExpression> + Send + Sync>> = {
        ...
        register_function!(m, "<", make_const_meta_body!(DataType::Boolean), make_binary_primitive_array_evaluate_function!(lt));
        register_function!(m, "<=", make_const_meta_body!(DataType::Boolean), make_binary_primitive_array_evaluate_function!(lt_eq));
        register_function!(m, "=", make_const_meta_body!(DataType::Boolean), make_binary_primitive_array_evaluate_function!(eq));
        register_function!(m, ">=", make_const_meta_body!(DataType::Boolean), make_binary_primitive_array_evaluate_function!(gt_eq));
        register_function!(m, ">", make_const_meta_body!(DataType::Boolean), make_binary_primitive_array_evaluate_function!(gt));
        register_function!(m, "upper", make_const_meta_body!(DataType::Utf8), Arc::new(|args: Vec<ArrayRef>| {
            let output: Result<_, ArrowError> = compute_single_arg!(args[0], StringArray, StringBuilder, |text: &str| {
                text.to_uppercase()
            });
            Ok(output? as ArrayRef)
        }));
